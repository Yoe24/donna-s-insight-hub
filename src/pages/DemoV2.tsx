...
import { Settings, LayoutDashboard, Paperclip, Eye, Edit3, Send, ChevronRight, Mail, ArrowUp } from "lucide-react"
...
function getDemoResponse(q: string): string {
  ...
  // SCI Les Tilleuls / loyers / Benzara
...
function DonnaChatPanel() {
  ...
  return (
    <div style={{ width: 380, flexShrink: 0, borderLeft: `1px solid ${BORDER}`, display: "flex", flexDirection: "column", background: BG, height: "100%" }}>

      {/* Header */}
      <div style={{ padding: "16px 20px", borderBottom: `1px solid ${BORDER}`, display: "flex", alignItems: "center", gap: 10, background: BG, flexShrink: 0 }}>
        ...
        <div style={{ marginLeft: "auto", fontSize: 11, color: TEXT_LIGHT, background: "#F9FAFB", padding: "3px 8px", borderRadius: 6, border: `1px solid ${BORDER}` }}>
          Cmd+K
        </div>
      </div>
                ...
                <div style={{ background: "#F9FAFB", borderRadius: "16px 16px 16px 4px", padding: "10px 14px", fontSize: 13, color: TEXT, lineHeight: 1.65, border: `1px solid ${BORDER}` }}>
                  <ReactMarkdown
                    components={{
                      ...
                      strong: ({ children }) => <strong style={{ fontWeight: 600, color: TEXT }}>{children}</strong>,
                      em: ({ children }) => <em style={{ color: TEXT_MUTED }}>{children}</em>,
                      ...
                      th: ({ children }) => <th style={{ padding: "4px 8px", border: `1px solid ${BORDER}`, background: "#F3F4F6", fontSize: 11, fontWeight: 600 }}>{children}</th>,
                      td: ({ children }) => <td style={{ padding: "4px 8px", border: `1px solid ${BORDER}` }}>{children}</td>,
                      hr: () => <hr style={{ border: "none", borderTop: `1px solid ${BORDER}`, margin: "8px 0" }} />,
                      code: ({ children }) => <code style={{ background: "#F3F4F6", padding: "1px 4px", borderRadius: 3, fontSize: 12 }}>{children}</code>,
                    }}
                  >
                    {msg.content}
                  </ReactMarkdown>
      ...
      {/* Input */}
      <div style={{ padding: "12px 16px 16px", borderTop: `1px solid ${BORDER}`, background: BG, flexShrink: 0 }}>
      ...
      <div style={{ flex: 1, display: "flex", overflow: "hidden" }}>
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "32px 32px" }}>
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 30, fontWeight: 400, color: TEXT, marginBottom: 4, letterSpacing: "-0.02em" }}>Bonjour, Alexandra</h1>
            <p style={{ fontSize: 13, color: TEXT_MUTED, marginBottom: 24 }}>Jeudi 3 avril</p>
          </motion.div>

          <AnimatePresence>
            {statsVisible && (
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} style={{ border: `1px solid ${BORDER}`, borderRadius: 12, padding: "18px 22px", marginBottom: 16, display: "flex", alignItems: "center", gap: 20 }}>
                <div style={{ textAlign: "center" }}>
                  <div style={{ fontSize: 32, fontWeight: 700, color: TEXT, lineHeight: 1 }}>0</div>
          ...
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }} style={{ border: `1px solid ${BORDER}`, borderRadius: 12, padding: "18px 22px", marginBottom: 24 }}>
            {donnaTyping ? (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1.5, duration: 0.6 }} style={{ marginTop: 24, padding: "18px 22px", borderRadius: 12, background: ACCENT_BG, border: `1px solid rgba(37,99,235,0.15)`, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 12 }}>
            <div>
